...
function Database(databaseName, openCallback) {
  ...
  NativeModules.ReactNativeSpatialite.openFromFilename(databaseName, (error, databaseId) => {
    if (error) {
...
Database.prototype = {
  ...
  executeSQL (
    ...
    this._addAction(completeCallback, (callback) => {
      NativeModules.ReactNativeSpatialite.prepareStatement(this._databaseId, sql, params, (error, statementId) => {
        if (error) {
          completeCallback(new SQLite3Error(error));
          return;
        }
        var next = () => {
          NativeModules.ReactNativeSpatialite.stepStatement(this._databaseId, statementId, (error, row) => {
            if (error) {
              completeCallback(new SQLite3Error(error));
              return;
            }
  ...
  close(callback: ?(error: ?SQLite3Error) => void) {
    NativeModules.ReactNativeSpatialite.closeDatabase(this._databaseId, (error) => {
      if (!callback) return;
      if (error) {
...
export const ReactNativeSpatialite = {
